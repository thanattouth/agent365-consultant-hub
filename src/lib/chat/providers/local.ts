...
import { consultantModeById } from "@/lib/chat/modes";
...
export const localChatProvider: ChatAnswerProvider = {
  id: "local",
  async draftResponse(input) {
    return draftLocalConsultantResponse(input);
  },
};

function draftLocalConsultantResponse({
  message,
  mode,
